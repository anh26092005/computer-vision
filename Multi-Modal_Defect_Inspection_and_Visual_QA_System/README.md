...
### 🖼️ Phân tích Ảnh tĩnh (Image Inspection)
- Chế độ chọn nhiều ảnh (`.png`, `.jpg`, ...) để kiểm tra thủ công.
- Slideshow phân tích và hiển thị kết quả trực tiếp cho từng ảnh với các thông tin chi tiết (Lỗi gì, Tỷ lệ %).
...
```plaintext
📦 Multi-Modal_Defect_Inspection_and_Visual_QA_System
 ┣ 📂 core/
 ┃  ┣ 📜 losses.py       # Định nghĩa các hàm mất mát (Dice, BCE, CrossEntropy)
 ...
 ┣ 📂 extracted_data/    # Dữ liệu đã trích xuất/xử lý
 ┣ 📜 app.py             # 🖥️ Giao diện Desktop (Tkinter) chính
 ┣ 📜 train.py           # Script huấn luyện và vẽ biểu đồ metrics
 ┣ 📜 evaluate.py        # Script đánh giá hiệu năng mô hình
 ┣ 📜 check_data.py      # Script kiểm tra dữ liệu đầu vào
 ┣ 📜 extract_frames.py  # Script trích xuất khung hình từ video
